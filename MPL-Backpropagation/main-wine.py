...
table.add_row([460,0.1,0,0.7,ret['err'],ret['precisao']])
ret = wine_test(eta=0.3)
table.add_row([460,0.3,0,0.7,ret['err'],ret['precisao']])
ret = wine_test(eta=0.5)
table.add_row([460,0.5,0,0.7,ret['err'],ret['precisao']])

# Variação do número de ciclos (épocas)
print('\n\n### Testando Numero de Ciclos ###')
ret = wine_test(max_iter=300)
table.add_row([300,0.1,0,0.7,ret['err'],ret['precisao']])
ret = wine_test(max_iter=750)
table.add_row([650,0.1,0,0.7,ret['err'],ret['precisao']])
...
ret = wine_test(train_size=0.4)
table.add_row([550,0.1,0,0.5,ret['err'],ret['precisao']])
ret = wine_test(train_size=0.5)
table.add_row([550,0.1,0,0.6,ret['err'],ret['precisao']])
ret = wine_test(train_size=0.7)
table.add_row([550,0.1,0,0.9,ret['err'],ret['precisao']])

# Variação do Momentum
print('\n\n### Testando Momento ###')
ret = wine_test(alpha=0.2)
table.add_row([500,0.1,0.1,0.7,ret['err'],ret['precisao']])
ret = wine_test(alpha=0.5)
