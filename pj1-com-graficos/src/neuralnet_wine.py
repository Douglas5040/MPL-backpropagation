...
# modules used for reading the dataset and spliting it in stratifid sets
import pandas as pd
...
class NeuralNet():
    ...
    def train(self, X, y, n_epochs, l_rate, momentum):
        ...
        for ep in range(n_epochs):
            ...
            error = np.mean(np.abs(y - self.y_hat))
...
for i in range(len(test_set_size)):
    # Get Data
    X_train, X_test, y_train, y_test = NNData().get_wine_data(test_set_size[i])
...
plt.title("Acurácia para Diferentes Proporções de Treino e Teste")
plt.axis([test_set_size[0], test_set_size[-1], 0, 100])
plt.tight_layout()
ax.legend()
plt.grid()
plt.show()
...
plt.title("Acurácia para Diferentes Quantidades de Épocas")
...
plt.ylabel('Acurácia (%)')
plt.xlabel('Momentum')
plt.title("Acurácia para Diferentes Momentums")
plt.axis([.25, .85, 0, 100])
plt.tight_layout()
...
plt.title("Acurácia para Diferentes Taxas de Aprendizagem")
plt.axis([.01, 3, 0, 100])
plt.tight_layout()
...
"""
fig, ax = plt.subplots()
plt.plot(range(0, len(nn.errors_)), nn.errors_)
plt.xlabel('Epochs')
plt.ylabel('Root Mean Square Error')
plt.title("Root Mean Squared Error vs. Epochs")
plt.axis([0, len(nn.errors_), 0, 1])
labels = [str(x * 1000) for x in range(round(n_epochs / 100))]
ax.set_xticklabels(labels)
plt.tight_layout()
plt.grid()
plt.show()
